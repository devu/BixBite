package com.mindScriptAct.bixbite.notes {

/**
 * ...
 * 
 * @author Daniel Wasilewski Borrowed from:
 * @author Deril (raima156@yahoo.com)
 */
public class Note {
	static public const CALL_COMMANDS_EMPTY:String = "callCommandsEmpty";
	static public const CALL_COMMANDS_GET_PARAMS:String = "callCommandsGetParams";
	static public const CALL_COMMANDS_WITH_MODEL:String = "callCommandsWithModel";
	static public const CALL_COMMANDS_WITH_MODEL_COMM_VIEWS:String = "callCommandsWithModelCommViews";
	
	static public const CALL_MODEL:String = "callModel";
	static public const CALL_MODEL_COMM_VIEWS:String = "callModelCommViews";
	
	static public const OUTPUT_APPEND_LINE:String = "outputAppendLine";
	
	static public const CREATE_TEST_VIEW:String = "createTestView";
	static public const REMOVE_TEST_VIEW:String = "removeTestView";
	static public const ACTIVATE_MEDIATOR:String = "activateMediator";
	
	static public const MEDIATOR_COMM_TEST:String = "mediatorCommTest";

}
}